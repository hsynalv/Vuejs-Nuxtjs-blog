    ...
    <nuxt-link class="overflow-hidden " :to="postLink">
      ...
      <div class="p-3">
        ...
        <p class="mt-2 text-[12px] my-3 flex flex-wrap -m-1 text-overflow">
          <a @click.prevent="$emit('changeCurrentTag', tag)" href="" v-for="tag in tags" :key="tag" class="m-1 leading-loose text-slate-400 border border-current lowercase px-2 rounded font-medium">#{{tag}}</a>
        </p>
      </div>
  </div>
</template>
